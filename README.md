...
| `Device/` | 设备驱动层，包括 BMI088、舵机、语音、激光、按键、LED、蜂鸣器、WS2812、头部姿态分析和USART10无线桥接。 |
...
程序入口位于 `Core/Src/main.c`。启动后执行 HAL 初始化、系统时钟配置和外设初始化，然后依次初始化 BMI088、舵机、激光、按键、LED、头部姿态分析、训练状态机、蜂鸣器、WS2812、语音模块和USART10无线桥接。

初始化阶段包含几个硬件自检动作：

- `BMI088_init()` 循环等待 IMU 初始化成功，然后调用 `BMI088_AsyncStart()` 启动异步采样。
- 舵机初始化后固定停放在 X/Y 均为 `90°`；`Servo_Test()` 当前默认关闭，避免头盔上电时自动大范围摆动。
...
- `SYS_PAUSE`：训练过程中检测到异常姿态后进入暂停。暂停时关闭激光，舵机停放到 `90°/90°`，语音和蜂鸣器提示患者调整姿态。姿态连续正常 3 秒后自动恢复训练，并补偿暂停期间的训练时间基准。
...
| `MODE_D_FOCUS` | 视觉聚焦训练 | 单激光目标点亮后等待患者按 PA2 确认，记录正确、超时和反应时间；当前硬件不具备自动近远调焦能力。 |
...
uint8_t CALIB_Y_MIN = 75;
uint8_t CALIB_Y_MAX = 135;
```

上电后系统默认使用 X 轴 `80°～110°`、Y 轴 `85°～95°` 的窄训练范围，X/Y 初始角均为 `90°`。固定机械硬保护范围为 `10°～170°`。系统不强制进入舵机视野标定；需要重新测量佩戴后的视野范围时，可通过语音或手持屏“标定模式”触发。

1. X 轴从 30 度扫描到 150 度，按 PA2 记录一个边界。
2. X 轴从 150 度扫描回 30 度，按 PA2 记录另一个边界。
3. Y 轴从 75 度扫描到 135 度，按 PA2 记录一个边界。
4. Y 轴从 135 度扫描回 75 度，按 PA2 记录另一个边界。
5. 四个点都记录完成后，自动排序 min/max，更新 `CALIB_X/Y_MIN/MAX`，舵机回到 `90°/90°` 安全停放位。
...
本次运行后续训练使用新的范围和中心生成刺激点位；只有真正进入训练后才使用配置中心。上电、空闲、暂停和停止均固定停放在 `90°/90°`。
...
- 标定结果保存在 RAM，本次运行后续训练使用新的范围和中心生成点位；空闲、暂停和停止仍固定停放在 `90°/90°`。四个边界点不足时不会更新范围，会自动重新标定。
- 语音“标定模式”可在任意状态触发，完成后回到模式选择。

## 无线手持屏控制

当前工程已经接入 `ESP32-S3-Touch-LCD-5 + XIAO ESP32C3` 无线手持控制器：

```text
5寸触摸屏 -- ESP-NOW --> XIAO ESP32C3 -- USART10 --> STM32H723
```

- 屏幕端包含训练控制、参数设置、训练报告和连接测试四页。
- 完整训练固定执行五项；自选训练可选择1至5项并按选择顺序连续执行。
- 基础控制包括开始、暂停、继续、停止和舵机视野标定；语音与实体按键仍然并行可用。
- 开始训练后继续使用原有姿态校准流程，必须按 PA2 确认；PA2仍用于患者反馈和完整模式下一项确认。
- 屏幕暂停属于人工保持暂停，只有屏幕“继续”或原有语音恢复命令才能恢复；姿态异常安全暂停不能由屏幕强制解除，姿态稳定3秒后自动恢复。
- 停止命令会立即关闭激光、关闭聚焦LED、舵机回到 `90°/90°`，将当前记录标记为未完成并返回空闲。

STM32使用 `USART10` 与XIAO通信：

```text
PE3 / USART10_TX -> XIAO D7 / GPIO20 / RX
PE2 / USART10_RX <- XIAO D6 / GPIO21 / TX
GND              -  GND
```

串口参数为 `115200 8N1`，RX使用DMA和空闲中断。USART1继续用于VOFA调试输出，UART7继续连接CI1302语音模块。

无线协议版本4继续使用固定44字节帧、CRC16、固定MAC和ESP-NOW信道1。新增舵机预览及确认帧；配置和控制命令必须收到STM32 ACK。STM32每200ms回传训练状态。

参数页支持初级、标准、进阶预设和逐项精确调整。长按“舵机测试”可在激光强制关闭的条件下，以 `1°` 步进预览 X/Y 实体位置并记录最小、初始和最大角；固定范围为 `10°～170°`，X/Y 最小跨度分别为 `10°/5°`。配置保存在ESP32-S3 `Preferences`，重连后同步到STM32。

训练结束后屏幕主动查询会话摘要和各模式报告，并在FFat中循环保存最近20次。XIAO为报告/ACK设置独立高优先级发送队列和缓存重发，屏幕使用独立报告接收队列；连接测试页显示查询、串口返回及ESP-NOW成功/失败计数。20份满后循环覆盖最旧记录，不影响刷新。

详细的Arduino环境、烧录顺序、供电接线、协议字段和故障排查见 `Wireless/README.md`。

当前版本尚未接入云服务器、小程序或账号系统；本地报告结构和会话编号已为后续云端上传预留。

屏幕端使用双帧缓存整帧刷新，启动时先清空两块RGB565帧缓存；四个页签使用不透明背景。报告和参数写入FFat/Preferences时先退出LVGL锁，避免刷新报告、恢复默认或RESET后出现旧页面叠加。

训练控制页的“标定模式”仅在空闲状态可用。屏幕发起后，PA2确认开始并依次记录X轴两个、Y轴两个边界；完成后STM32更新本次上电使用的范围和中心角，屏幕自动读取并显示新参数。STM32重新上电后应重新标定。
