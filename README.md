...
上电后第一次选择完整模式或自选模式，并播放进入模式提示后，如果本次运行尚未完成舵机范围标定，系统会先进入 `SYS_SERVO_CALIB_READY=11`，播放 `VOICE_TTS_SERVO_CALIB_START=0x2E`，提示保持头部居中并等待 PA2 确认。按下 PA2 后进入 `SYS_CALIB_SERVO`，打开激光并按顺序扫描 X/Y 轴：
...
四个边界点必须全部由 PA2 记录。若任意阶段没有按键，系统不会使用默认兜底值通过标定，而是在完成阶段播放 `VOICE_TTS_CALIB_POINTS_LOW=0x31`，提示“标定点不足，重新标定”，然后清空本轮记录并从 X 轴 70→150 重新开始。标定值保存在 RAM 中，设备重启后恢复代码默认值。
...
PA2 单击完整模式或双击自选模式后，系统会进入 `SYS_MODE_ENTER_PROMPT=9`，播放对应进入提示，并固定等待约 2.5 秒。若本次上电尚未完成舵机视野标定，会先进入 `SYS_SERVO_CALIB_READY=11`，按 PA2 完成激光范围标定后再继续；完整模式随后进入 `SYS_CALIBRATE`，自选模式随后进入 `SYS_IDLE_VOICE` 等待语音选择训练。

这些文件通常由本地编译生成，体积较大，且容易因为重新编译产生无关差异。除非团队明确要求保存固件产物，否则推荐只提交源码、配置文件和文档。

## 最近一次改动说明：佩戴后舵机视野标定

- 新增 `SYS_SERVO_CALIB_READY=11`，进入完整/自选模式后的首次流程会先提示患者保持头部居中，并按 PA2 开始激光范围标定。
