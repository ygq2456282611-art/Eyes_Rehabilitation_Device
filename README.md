...
- `SYS_SERVO_CALIB_READY`：舵机视野标定准备状态，播放 `VOICE_TTS_SERVO_CALIB_START=0x2E`，等待 PA2 确认后进入扫描标定。
- `SYS_CALIB_SERVO`：舵机视野扫描标定状态，用 PA2 按键标记 X/Y 视野边界，更新训练用舵机角度范围。

语音命令分为两类：

- 命令词：选择训练模式、模式切换或进入标定模式。五种训练模式名当前在空闲、训练、暂停和反馈状态下都可用于直接切换到指定模式；“标定模式”可在任意状态重新触发舵机视野标定。
...
上电后选择完整模式或自选模式，并播放进入模式提示后，系统默认直接使用代码中的 `CALIB_X/Y_MIN/MAX` 训练范围，不再强制进入舵机视野标定。需要重新测量佩戴后的视野范围时，可通过语音命令“标定模式”进入 `SYS_SERVO_CALIB_READY=11`，播放 `VOICE_TTS_SERVO_CALIB_START=0x2E`，提示保持头部居中并等待 PA2 确认。按下 PA2 后进入 `SYS_CALIB_SERVO`，打开激光并按顺序扫描 X/Y 轴：

1. X 轴从 30 度扫描到 150 度，按 PA2 记录一个边界。
2. X 轴从 150 度扫描回 30 度，按 PA2 记录另一个边界。
3. Y 轴从 80 度扫描到 140 度，按 PA2 记录一个边界。
4. Y 轴从 140 度扫描回 80 度，按 PA2 记录另一个边界。
5. 四个点都记录完成后，自动排序 min/max，更新 `CALIB_X/Y_MIN/MAX`，舵机回到当前标定范围中心。

四个边界点必须全部由 PA2 记录。若任意阶段没有按键，系统不会使用默认兜底值通过标定，而是在完成阶段播放 `VOICE_TTS_CALIB_POINTS_LOW=0x31`，提示“标定点不足，重新标定”，然后清空本轮记录并从 X 轴 30→150 重新开始。标定值保存在 RAM 中，设备重启后恢复代码默认值。

本次运行后续训练都会使用新的 `CALIB_X/Y_MIN/MAX` 生成扫视、追踪和空间忽略点位；暂停、停止刺激和训练结束回中不再固定使用 `90,90`，而是使用 `(CALIB_X_MIN + CALIB_X_MAX) / 2` 与 `(CALIB_Y_MIN + CALIB_Y_MAX) / 2`。

语音命令“标定模式”可在任意状态触发。触发后会停止当前刺激、丢弃当前训练进度并进入 `SYS_SERVO_CALIB_READY`；标定完成后回到 `SYS_MODE_SELECT`，让用户重新选择完整模式或自选模式。
...
本轮将训练入口拆成两种流程：完整模式和自选模式。系统初始化并播放欢迎提示后，不再停在 PA2 测试关卡，而是进入 `SYS_MODE_SELECT` 模式选择状态，播放 `VOICE_TTS_MODE_SELECT=0x25`。在该状态下，PA2 单击进入完整训练模式，PA2 双击进入自选模式；双击只在模式选择状态内判断，不影响训练中的 PA2 确认。

完整模式使用固定顺序执行五项训练：

```text
注视稳定训练 -> 扫视训练 -> 平稳追踪训练 -> 视觉聚焦训练 -> 空间忽略训练
```

每一项训练的开始流程统一为：进入 `SYS_CALIBRATE` 播放 `VOICE_TTS_START_CALIB=0x24`，等待 PA2 确认姿态校准；校准完成后播放 `VOICE_TTS_CALIB_DONE=0x01`，进入 `SYS_TRAIN_PROMPT` 播放对应训练开始提示，并固定等待约 4 秒后再进入 `SYS_TRAIN`。这 4 秒提示等待不会计入训练时间。

五个训练开始提示 ID 如下：

| 训练模式 | 播报 ID | 协议 |
| --- | --- | --- |
| 注视稳定训练 | `0x27` | `AA 55 FF 27 FB` |
| 扫视训练 | `0x28` | `AA 55 FF 28 FB` |
| 平稳追踪训练 | `0x29` | `AA 55 FF 29 FB` |
| 视觉聚焦训练 | `0x2A` | `AA 55 FF 2A FB` |
| 空间忽略训练 | `0x2B` | `AA 55 FF 2B FB` |

完整模式中，每项训练结束后保留原有单项反馈播报。若不是最后一项，则进入 `SYS_NEXT_CONFIRM`，播放 `VOICE_TTS_NEXT_CONFIRM=0x26`，等待 PA2 确认后进入下一项的手动校准。完成空间忽略训练后播放 `VOICE_TTS_ALL_DONE=0x16`，然后回到模式选择。

自选模式下，系统回到 `SYS_IDLE_VOICE` 等待语音选择五种训练之一。语音选中训练后，同样执行“手动校准 -> 训练开始提示 -> 固定等待 -> 训练 -> 单项反馈”的流程。单项反馈后回到自选等待，不自动进入下一项，也不播放全部完成提示。

新增普通命令词 `VOICE_CMD_MODE_SWITCH=0x08`，对应协议表里的“模式切换”。收到该命令后会从空闲、训练、暂停或反馈状态进入 `SYS_MODE_SELECT`。暂停/继续训练继续优先使用唤醒词区命令 `TYPE=0x01` 和 `TYPE=0x06`，普通 `TYPE=0x00, ID=0x08` 不再作为暂停/继续处理，避免和“模式切换”冲突。

VOFA 中 `AppState` 新增状态值：

| AppState | 含义 |
| --- | --- |
| `6` | `SYS_MODE_SELECT`，模式选择 |
| `7` | `SYS_TRAIN_PROMPT`，训练开始提示等待 |
| `8` | `SYS_NEXT_CONFIRM`，完整模式等待 PA2 进入下一项 |
| `9` | `SYS_MODE_ENTER_PROMPT`，进入完整/自选模式提示等待 |
| `10` | `SYS_MODE_CMD_ACK_WAIT`，训练模式命令短确认等待 |
| `11` | `SYS_SERVO_CALIB_READY`，舵机视野标定准备，等待 PA2 开始扫描 |

`last_tx_id` 可用于确认语音播报是否正确：模式选择为 `37`，确认下一个模式为 `38`，开始校准为 `36`，五个训练开始提示依次为 `39~43`，进入完整/自选模式为 `44/45`，舵机视野标定开始提示为 `46`，标定中按键记录点提示为 `47/48`，点数不足重新标定提示为 `49`。

`last_app_event` 新增事件值：
...
模式选择后新增进入成功提示，避免 PA2 选择后立刻进入校准导致播报重叠：

| 播报 | ID | 协议 | VOFA `last_tx_id` |
| --- | --- | --- | --- |
| 成功进入完整训练模式 | `0x2C` | `AA 55 FF 2C FB` | `44` |
| 成功进入自选模式 | `0x2D` | `AA 55 FF 2D FB` | `45` |

PA2 单击完整模式或双击自选模式后，系统会进入 `SYS_MODE_ENTER_PROMPT=9`，播放对应进入提示，并固定等待约 2.5 秒。当前版本不会在进入完整/自选模式后自动执行舵机视野标定，而是先使用默认训练范围；完整模式随后进入 `SYS_CALIBRATE`，自选模式随后进入 `SYS_IDLE_VOICE` 等待语音选择训练。需要舵机视野标定时，使用语音“标定模式”单独触发。

这些文件通常由本地编译生成，体积较大，且容易因为重新编译产生无关差异。除非团队明确要求保存固件产物，否则推荐只提交源码、配置文件和文档。

## 最近一次改动说明：佩戴后舵机视野标定

- 新增 `SYS_SERVO_CALIB_READY=11`，当前由语音“标定模式”触发，提示患者保持头部居中，并按 PA2 开始激光范围标定。
- 新增 `VOICE_TTS_SERVO_CALIB_START=0x2E`、`VOICE_TTS_CALIB_PRESS_POINT=0x2F`、`VOICE_TTS_CALIB_PRESS_AGAIN=0x30`、`VOICE_TTS_CALIB_POINTS_LOW=0x31`，VOFA 中 `last_tx_id=46~49` 可观察标定播报。
- 标定结果保存在 RAM，本次运行后续训练都使用 `CALIB_X/Y_MIN/MAX` 生成点位，并使用范围中心代替固定 `90,90` 回中；四个边界点不足时不会更新范围，会自动重新标定。
- 语音“标定模式”可在任意状态触发，完成后回到模式选择。
