...
`last_tx_id` 可用于确认语音播报是否正确：模式选择为 `37`，确认下一个模式为 `38`，开始校准为 `36`，五个训练开始提示依次为 `39~43`。
...
PA2 单击完整模式或双击自选模式后，系统会进入 `SYS_MODE_ENTER_PROMPT=9`，播放对应进入提示，并固定等待约 2.5 秒。完整模式随后进入 `SYS_CALIBRATE`，自选模式随后进入 `SYS_IDLE_VOICE` 等待语音选择训练。
