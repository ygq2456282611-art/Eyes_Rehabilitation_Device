 ...
 * @file    buzzer.c
 * @brief   板载蜂鸣器驱动实现
 *          使用 TIM12_CH2 (PB15) 输出 PWM 驱动蜂鸣器
 *
 *          时钟树：
 *          APB1 = 120MHz, TIM12 时钟 = APB1 × 2 = 240MHz
 *          Prescaler = 24-1 → 10MHz
 *          Period = 2000-1 → PWM 频率 = 10MHz / 2000 = 5kHz
 *          CCR2 控制占空比：0=静音, 500=50%, 1000=100%
 */
#include "buzzer.h"
